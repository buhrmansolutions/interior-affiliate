...
export default () => {
    ...
    const onFromInput = (e: any) => {
        ...
        document.getElementById("range")!.style.width = (100 * (to - newFromValue) / (maxPrice - minPrice)) + "%"
    ...
    const onToInput = (e: any) => {
        ...
        document.getElementById("range")!.style.width = (100 * (newToValue - from) / (maxPrice - minPrice)) + "%"
        setTo(newToValue)
    }
    return (
        <div>
            <div id="slider-distance">
                <div>
                    ...
                    <span id="from-handle" className="thumb" style={{ left: "0%" }}></span>
                    <span id="to-handle" className="thumb" style={{ left: "100%" }}></span>

                </div>
                <input id="from-input" type="range" value={from} max={maxPrice} min={minPrice} step={steps} onInput={onFromInput} />

                <input id="to-input" type="range" value={to} max={maxPrice} min={minPrice} step={steps} onInput={onToInput} />
            </div>
            <div className="label">
                Price: {from}{currency} - {to}{currency}
            </div >
        </div >
    )
}